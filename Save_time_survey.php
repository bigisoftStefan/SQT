...
// For each response the individual survey time
$response_time = array();

// Original response time array
$response_time_original = array(array());
...
// Get the response time for all responses, sorted in a descending manner (the longest duration are on the top of the sort)
$statement = $db_link->prepare("SELECT id, respond_speeding FROM Responses WHERE survey_id = ? and respond_speeding > 0 ORDER BY respond_speeding DESC");
$statement->bind_param("i", $survey_id);
$statement->execute();

if (!($statement->errno))
{
	$statement->bind_result($db_id, $time);
	
	$runner = 0;
	
	while ($statement->fetch()) 
	{
		// Array of each response which has a response time greater than 0
    	$response_time[$runner] = $time;
    	$response_time_original[$runner][0] = $db_id;
    	$response_time_original[$runner][1] = $time;
	...
	$statement = $db_link->prepare("SELECT Count(respond_speeding) FROM Responses WHERE survey_id = ? and respond_speeding > 0");
	$statement->bind_param("i", $survey_id);
	$statement->execute();
	...
	if (!($statement->errno))
	{
		...
		// First of all disregard the top 5% quantile
		$quantile =  intval(($response_count / 100) * 5);

		if($quantile >= count($response_time))
		{
			$quantile = count($response_time) - 1;
		}
		
		$response_count = $response_count - $quantile;
		
		for($i=$quantile; $i < count($response_time); $i++)
		{
			$sum_time = $sum_time + $response_time[$i];
		}
		...
		for($i = 0; $i < count($response_time_original); $i++)
		{
			// Compares the original values of the array to the calculated mean time
			$save_time = 0;
			
			// Index of 0 means speeding is detected and an index of 1 no speeding 
			// is detected.
			// Index will be saved as integer value between 0 and 100 %
		
			if($response_time_original[$i][1] < $mean_time_value)
			{
				$save_time = intval(($response_time_original[$i][1]/$mean_time_value)*100);
			}
			else
			{
				$save_time = 100;
			...
			$data_time [$i][0] = $response_time_original[$i][0];
			$data_time [$i][1] = $save_time;
		}		
